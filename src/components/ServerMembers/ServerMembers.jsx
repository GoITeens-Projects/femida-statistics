...
export const ServerMembers = () => {
   ...
   const statistics = useSelector(selectServerMembers); 
    ...
    const savedClickStates = JSON.parse(localStorage.getItem('serverMembersClickStates')) || {
        clickTotal: true,
        clickJoined: true,
        clickLeft: true,
    };
    ...
    const [hideStates, setHideStates] = useState({ total: false, joined: false, left: false });
            ...
            <div>
                ...
                <div className={styles.containerBorder}>
                    <div className={styles.containers}>
                        ...
                        >
                            <span className={styles.statusChart_Span}></span>
                            <p className={`${styles.totalMembersText} ${!clickStates.clickTotal ? styles.statusChart__statusItem__text__inactive : ''}`}>
                                Кількість учасників
                            </p>
                        ...
                        <AreaChart width={size} height={310} data={statistics}
                            ...
                            <defs>
                                <linearGradient id="colorTotal" x1="0" y1="0" x2="0" y2="1">
                                    <stop offset="5%" stopColor="var(--chart-accent-color)" stopOpacity={0.8} />
                                    <stop offset="95%" stopColor="var(--chart-accent-color)" stopOpacity={0.45} />
                                </linearGradient>
                            ...
                            {clickStates.clickTotal && (
                                <Area type="monotone" dataKey="total" stroke="var(--chart-accent-color)"
                                    fillOpacity={1} fill="url(#colorTotal)" name="Загальна кількість учасників"
                                    className={styles[hideStates.total ? 'fadeOut' : 'fadeIn']} isAnimationActive={true}
                                    animationDuration={1500} />
                            )}
                            {clickStates.clickJoined && (
                                <Area type="monotone" dataKey="joined" stroke="var(--btn-accent-color)"
                                    fillOpacity={1} fill="url(#colorJoined)" name="Приєдналося"
                                    className={styles[hideStates.joined ? 'fadeOut' : 'fadeIn']} isAnimationActive={true}
                                    animationDuration={1500} />
                            )}
                            {clickStates.clickLeft && (
                                <Area type="monotone" dataKey="left" stroke="var(--legend-brighter-teal-color)"
                                    fillOpacity={1} fill="url(#colorLeft)" name="Покинуло"
                                    className={styles[hideStates.left ? 'fadeOut' : 'fadeIn']} isAnimationActive={true}
                                    animationDuration={1500} />
                            )}
                        </AreaChart>
                    </div>
