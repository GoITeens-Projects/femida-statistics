  ...
  return (
    <section>
      <div>
        ...
        <p className={styles.statusChart__description}>
        ...
        <div className={styles.statusChart__border}>
          ...
          <div className={styles.statusChart__membersSummary}>
            {['online', 'away', 'dnd', 'offline'].map((status, index) => (
              <div
                key={status}
                className={`${styles[`statusChart__statusItem__${status}`]} ${
                  !clickStates[
              ...
              >
                ...
                <p
                  className={`${styles.statusChart__statusLabel} ${
                    !clickStates[
                      `click${status.charAt(0).toUpperCase() + status.slice(1)}`
                    ]
                ...
                <p
                  className={`${styles.statusChart__statusCount} ${
                    !clickStates[
                      `click${status.charAt(0).toUpperCase() + status.slice(1)}`
                    ]
            ...
            >
              <defs>
                <linearGradient id="colorOnline" x1="0" y1="0" x2="0" y2="1">
                  <stop
                    offset="5%"
                    stopColor="var(--chart-online-accent-color)"
                    stopOpacity={0.8}
                  />
                  <stop
                    offset="95%"
                ...
                <linearGradient id="colorAway" x1="0" y1="0" x2="0" y2="1">
                  <stop
                    offset="5%"
                    stopColor="var(--chart-inactive-color)"
                    stopOpacity={0.8}
                  />
                  <stop
                    offset="95%"
                ...
                <linearGradient id="colorDnd" x1="0" y1="0" x2="0" y2="1">
                  <stop
                    offset="5%"
                    stopColor="var(--chart-offline-accent-color)"
                    stopOpacity={0.8}
                  />
                  <stop
                    offset="95%"
              ...
              <XAxis
                dataKey="name"
                tick={{ fontSize: 12, stroke: '#666' }}
                axisLine={{ stroke: '#ccc', strokeWidth: 2 }}
                tickLine={{ stroke: '#ccc' }}
              />
              <YAxis
                axisLine={false}
                tick={{ stroke: '#666', dx: -3 }}
                tickLine={false}
                ...
                status =>
                  clickStates[
                    ...
                    <Area
                      ...
                      stroke={
                        status === 'online'
                          ? 'var(--chart-online-accent-color)'
                          : status === 'away'
                          ? 'var(--chart-inactive-color)'
                          : status === 'dnd'
                          ? 'var(--chart-offline-accent-color)'
                          : '#999999'
                      }
                      fillOpacity={1}
                      fill={`url(#color${
                      ...
                      className={
                        styles[hideStates[status] ? 'fadeOut' : 'fadeIn']
                      }
