...
import styles from './AutoModerationFilter.module.css';
...
export const AutoModerationFilter = ({
  moderationTitle,
  moderationList,
  endpoint,
  nameForUpdate,
  ...
  useEffect(() => {
    if (settings?.settings?.[nameForUpdate]?.enabled !== undefined) {
      setIsEnabled(settings.settings[nameForUpdate].enabled);
    }
  }, [settings, nameForUpdate]);
