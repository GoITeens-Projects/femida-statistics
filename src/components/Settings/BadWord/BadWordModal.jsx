...
export const BadWordPage = () => {
  ...
  const {
    ...
    loading,
    error,
  } = useSelector(state => state.settings); // Отримання налаштувань з Redux
  ...
  const handleBackClick = e => {
    ...
    const isWordsChanged =
      JSON.stringify(settings?.settings?.badwords?.words) !==
      JSON.stringify(addedWords);
    const isMuteTimeChanged =
      settings?.settings?.badwords?.actions?.mute?.muteTimeMs !== muteTimeMs;
    const isGiveWarnChanged =
      settings?.settings?.badwords?.actions?.giveWarn !==
      (selectedAction === 'warning');
    const isDeleteMsgChanged =
      settings?.settings?.badwords?.actions?.deleteMsg !== isDeleteMessage;
    const isCheckedAdminChanged =
      settings?.settings?.badwords?.actions?.ignoreAdmins !== isCheckedAdmin;
    const isCheckedNotifyUserChanged =
      settings?.settings?.badwords?.actions?.notifyUser?.enabled !==
      isCheckedNotifyUser;
